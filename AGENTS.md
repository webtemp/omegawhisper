...
- Silence fed to Whisper makes it invent text. `holds_speech` is what stops that: a recording with no speech in it never reaches the model. `trim_quiet_edges` cuts the quiet start and end. Pauses in the middle are untouched unless Pause-shortening is on (off by default). Energy-based voice detection was removed in 0.2.1 — it fed the model 3.2x the audio, chopped into fragments, and filtered nothing.
